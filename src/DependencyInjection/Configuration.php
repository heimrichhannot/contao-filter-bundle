        ...
        $rootNode
            ->children()
                ->arrayNode('filter')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('disable_legacy_filters')
                            ->defaultFalse()
                            ->info('Disable legacy filters to be able to use new implementation of filters together with HTTP GET requests.')
                        ->end()
                        ->arrayNode('initial_types')
                            ->scalarPrototype()
                                ->defaultValue([])
                                ->info('Filter types that can be used as initial.')
                            ->end()
                        ->end()
                        ->arrayNode('types')
                            ->arrayPrototype()
